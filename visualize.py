...
def display_detection(image, boxes, masks, class_ids, class_names, source_id, outputpath,
                      scores=None,  title="",
    ...
    output_path_polypoints = outputpath + 'txt/'+source_id+'.txt'
    output = open(output_path_polypoints, 'w+')
    output.writelines(str(N) + '\n')
    for i in range(N):
        color = colors[i]
        ...
        #find the major polygon in one mask
        max_number = 0
        for poly in contours:
            # Subtract the padding and flip (y, x) to (x, y)
            poly = np.fliplr(poly) - 1
            if len(poly) >= max_number:
                max_number = len(poly)
            else:
                continue
        #output the polygons
        for verts in contours:
            ...
            n = len(verts)
            if len(verts) >= max_number:
                output.writelines(str(i) + ',' + str(len(verts)) + '\n')
                polys = np.zeros((n, 2), dtype=np.float32)
                for j, k in enumerate(verts):
                    outline = str(k[0]) + ',' + str(k[1]) + ','+str(score) + '\n'
                    output.writelines(outline)
                    polys[j, 1] = k[1]
                    polys[j, 0] = k[0]
                # masked_image = utils.draw_poly(polys, masked_image)
    ...
    file_name = 'img/'+source_id+'_mask.tif'
    # file_name = "building_{:%Y%m%dT%H%M%S}.png".format(datetime.datetime.now())
    output_path_image = outputpath+file_name
    skimage.io.imsave(output_path_image, masked_image.astype(np.uint8))
    # if auto_show:
    #     plt.show()
